...
from bugtracker.settings import SECRET_KEY
...
class testing(APIView):
    ...
    def get(self, request, pk=''):
        ...
        decoded_payload = jwt.decode('eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ0b2tlbl90eXBlIjoiYWNjZXNzIiwiZXhwIjoxNjI1MTk5Njc1LCJqdGkiOiIwMWJlZjYyNzY0YTk0Y2NlYjk4NmVkOTI3ZmQxMjA5OCIsInVzZXJfaWQiOjJ9._6S8jtA1WA_urKf4Rt-hiw-CEwXtPrT00WsMqx-Fzu4',SECRET_KEY,algorithms="HS256")
        print(decoded_payload)
        theuser = User.objects.get(id=decoded_payload['user_id'])
        print(theuser)
        '''
        if pk:
            allissues = IssuesSerializer(Issues.objects.filter(text__contains=pk),many=True).data

            return JsonResponse({'status':'success','data':allissues})
